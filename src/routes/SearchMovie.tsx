...
const SearchMovie = () => {
    ...
    useEffect(() => {
      ...
      const getMovies = async () => {
        const response = await tmdbService.get(`/search/movie?query=${searchParams.get("q")}&include_adult=false&language=pt-BR&page=${pages}`);
    ...
    const result = results?.results.filter((movie) => movie.poster_path !== null);
                    ...
                    {result?.length === 0 && pages === 1 ? <p className={style.feedback}>Não foram encontradas respostas para a sua pesquisa.</p> : result?.map((result) => (
                        <article key={result.id}>
                            ...
                            <div className={style.container_img}>
                                ...
                                <Link to={`/movie/${result.id}`} className={style.button}>Ver mais</Link>
                            </div>
                        </article>
                    ))}
                </div>
